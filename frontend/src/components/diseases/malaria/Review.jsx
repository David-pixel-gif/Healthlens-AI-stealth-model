// File: src/components/diseases/malaria/Review.jsx
import React, { useEffect, useState, useMemo } from "react";
import { useParams, Link } from "react-router-dom";

// ✅ This one is fine as long as services/http.js exists
    ...
    <Container className="my-4">
      <Card className="p-4 shadow-sm border-0 rounded-4">
        <Row className="align-items-center">
          <Col>
            <h3 className="mb-0">🦟 Malaria · Review</h3>
            <div className="text-muted">Diagnosis ID: {id}</div>
          </Col>
          <Col className="text-end">
            <Link to={`/diseases/malaria/treat/${id}`}>
            ...
            {current ? (
              ...
              <>
                ...
                <Card className="mt-3 border-0 shadow-sm">
                  <Card.Body>
                    <h5 className="mb-3">Recent Malaria Diagnoses</h5>
                    <RBTable hover responsive className="mb-0">
                      ...
                      <tbody>
                        {items
                          .filter((x) => x.disease_key === "malaria")
                          .map((x) => (
                            <tr key={x.id}>
                              <td>{x.id}</td>
                              ...
                              <td>{x.created_at || "—"}</td>
                              <td>
                                <Link to={`/diseases/malaria/review/${x.id}`}>
